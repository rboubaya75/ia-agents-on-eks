...
output "vector_indexes" {
  description = "All retained immutable vector index generations for migration and retirement workflows."
  value = {
    for generation, index in aws_s3vectors_index.documents : generation => {
      active     = var.vector_index_generations[generation].active
...
output "vector_index_name" {
  description = "Active immutable S3 Vectors index consumed by IA_VECTOR_INDEX_NAME."
  value       = aws_s3vectors_index.documents[local.active_vector_index_generation].index_name
}

output "vector_index_arn" {
  description = "Active immutable S3 Vectors index ARN for Phase 4C workload policies."
  value       = aws_s3vectors_index.documents[local.active_vector_index_generation].index_arn
}

output "vector_index_generation" {
  description = "Active immutable index generation identifier."
  value       = local.active_vector_index_generation
}

output "embedding_profile_alias" {
  description = "Active server-controlled embedding profile alias."
  value       = local.active_vector_index.embedding_profile_alias
}

output "embedding_profile_revision" {
  description = "Active immutable embedding profile revision."
  value       = local.active_vector_index.embedding_profile_revision
}

output "embedding_dimensions" {
  description = "Active embedding vector dimension consumed by IA_EMBEDDING_DIMENSIONS."
  value       = local.active_vector_index.embedding_dimensions
}
...
output "application_runtime_settings" {
  ...
  value = {
    ...
    IA_VECTOR_INDEX_NAME                             = aws_s3vectors_index.documents[local.active_vector_index_generation].index_name
    IA_EMBEDDING_PROFILE_ALIAS                       = local.active_vector_index.embedding_profile_alias
    IA_EMBEDDING_PROFILE_REVISION                    = local.active_vector_index.embedding_profile_revision
    IA_EMBEDDING_DIMENSIONS                          = tostring(local.active_vector_index.embedding_dimensions)
  }
}
