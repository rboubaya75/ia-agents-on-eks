...
output "document_kms_key_arn" {
  description = "Effective KMS key ARN used by document storage, or null with AES256."
  value       = module.storage.encryption.kms_key_arn
}

output "document_kms_alias_name" {
  description = "Compatibility KMS alias name, or null when no compatibility customer key is created."
