...
variable "encryption" {
  description = "Compatibility encryption default and optional capability-specific overrides. Omitted or null override entries resolve to the compatibility default."
  type = object({
  ...
  validation {
    condition = alltrue([
      for contract in concat(
        [var.encryption.compatibility_default],
        [
          for contract in values(try(var.encryption.capability_overrides, {})) :
          contract if contract != null
        ],
        ) : (
    ...
    error_message = "Each effective encryption contract must use AES256 without a key or aws:kms with exactly one valid created/existing key and a 7-30 day deletion window."
  }
}
