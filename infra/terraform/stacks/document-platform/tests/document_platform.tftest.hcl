...
run "phase_4b_compatible_names_and_outputs" {
  ...
  assert {
    ...
    error_message = "AES256 storage mode must expose no customer key ARN."
  }
}

run "explicit_null_overrides_resolve_compatibility_default" {
  command = plan
  ...
  assert {
    condition     = output.storage_encryption.mode == "AES256"
    error_message = "An explicit null storage override must resolve to the compatibility default."
  }

  assert {
    condition     = output.document_kms_key_arn == null
    error_message = "Null overrides must not create or expose a customer key."
  }
}
...
run "storage_specific_existing_key_override" {
  ...
  assert {
    condition     = output.document_kms_key_arn == "arn:aws:kms:eu-west-3:123456789012:key/11111111-2222-3333-4444-555555555555"
    error_message = "The Phase 4B-compatible document KMS output must expose the key actually used by storage."
  }
}
