...
from ia_application.documents import DocumentSourceStore, UnsupportedDocumentContentTypeError
...
class Utf8DocumentExtractor(TextExtractor):
    ...
    async def extract(self, document: Document) -> ExtractedDocument:
        ...
        if not payload or len(payload) > self._max_bytes:
            raise InvalidExtractionError("document source size is invalid")
        checksum = hashlib.sha256(payload).hexdigest()
        if checksum != document.source_checksum:
            raise InvalidExtractionError("document source checksum does not match metadata")
        try:
            text = payload.decode("utf-8", errors="strict")
        except UnicodeDecodeError as error:
            raise InvalidExtractionError("document source is not valid UTF-8") from error
        ...
        if not normalized:
            raise InvalidExtractionError("document source contains no indexable text")
