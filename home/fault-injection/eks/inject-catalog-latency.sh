...
NAMESPACE="catalog"
DEPLOYMENT="catalog"
BACKUP_DIR="$HOME/fault-injection/eks/manifests"
BACKUP_FILE="$BACKUP_DIR/catalog-original.yaml"
...
kubectl top pods -n $NAMESPACE 2>/dev/null | sed 's/^/    /' || echo "    Metrics not available"
...
echo ""
echo "Rollback:"
echo "  ~/fault-injection/eks/rollback-catalog-latency.sh"
